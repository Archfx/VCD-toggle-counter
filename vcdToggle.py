...
4
Refer to IEEE standard 2001
(http://inst.eecs.berkeley.edu/~cs150/ProtectedDocs/verilog-ieee.pdf)
7
Author Donald 'Paddy' McCarthy (C) March 2008
'''
...
class VCD(object):
    ...
    def textstats(self):
        ...
        for ref in sorted(self.reference2idcode.keys()):
            ...
            if stats.size == 1:
                ...
                total += stats.size
                for count, (one2zero, zero2one) in enumerate(zip(stats.one2zero, stats.zero2one)):
        ...
        header = "# TOGGLE REPORT: %g %%, %i / %i covered. %i up-only, %i down-only." % (updown/1.0/total*100, updown, total, uponly, downonly )
        body = "toggle={\n" + "\n".join(out) + '\n }'
        return header, body
    ...
    def vector_value_change(self, format, number, id):
        ...
        for digit, olddigit, one2zero, zero2one in zip(number, stats.value, stats.one2zero, stats.zero2one):
...
def drop_declaration(tokeniser, keyword):
    next(dropwhile(lambda x: x != "$end", tokeniser)) #modified
    ...
    for count,token in enumerate(tokeniser):
        if not vcd.enddefinitions:
            ...
            if c == '$':
                ...
                vcd.vector_value_change(format=c.lower(), number=rest, id=next(tokeniser))
            else:
